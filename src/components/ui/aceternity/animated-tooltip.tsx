          ...
          <AnimatePresence mode="wait">
            {hoveredIndex === idx && (
              ...
              >
                ...
                <div className="text-zinc-400 mt-1">{item.designation}</div>
              </motion.div>
            )}
          </AnimatePresence>
          <img
            src={item.image}
            alt={item.name}
            className="object-cover !m-0 !p-0 h-14 w-14 rounded-full border-2 border-white group-hover:scale-105 group-hover:z-30 relative transition duration-500"
          />
        </div>
      ))}
    </div>
